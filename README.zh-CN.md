...
缓存写入按 TTL 计价：5 分钟为输入价的 1.25 倍，1 小时为 2 倍。拆分来自 transcript
中的 `usage.cache_creation`；没有该字段的旧 transcript 全部按 5 分钟价计算。

Fast mode（`usage.speed: "fast"`，Opus 5 / 4.8）按标准价的 2 倍计费。

Sonnet 5 按标准价 $3/$15 列出；$2/$10 的优惠价持续到 2026-08-31，在此之前
Sonnet 5 的费用会略微偏高。
